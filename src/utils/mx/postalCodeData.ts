...
export function formatMexicoPostalCode(value: number) {
  return String(value).padStart(2, '0')
}

export const MX_POSTAL_CODES = Array.from(
  { length: 99 },
  (_, index) => index + 1
)
  .filter((code) => code < 17 || code > 19)
  .map(formatMexicoPostalCode)

export function getMexicoPostalFeatureCodes(feature: unknown) {
